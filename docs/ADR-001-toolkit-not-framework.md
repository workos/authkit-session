...
async function updateSession(request: NextRequest) {
  ...
  const { refreshed, session: validated } = await core.validateAndRefresh(session);
...
```typescript
interface SessionPersistence<TRequest, TResponse> { /* cookies */ }
interface InRequestSession<TRequest, TResponse> { /* context */ }
```
